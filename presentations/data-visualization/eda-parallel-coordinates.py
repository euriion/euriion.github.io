...
def create_eda_parallel_coordinates():
    ...
    ax.set_title('EDA: 평행 좌표를 활용한 고객 세그멘테이션\n"다차원 고객 특성 패턴 분석"', 
                fontsize=16, fontweight='bold', pad=20)
    ax.legend(title='고객 유형', fontsize=12, title_fontsize=12)
    ax.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig('images/eda-parallel-coordinates.png', dpi=300, bbox_inches='tight', 
                facecolor='white', edgecolor='none')
    plt.close()

if __name__ == "__main__":
    create_eda_parallel_coordinates()
    print("EDA 평행 좌표 그래프 생성 완료")